...
public class LDFCommunityDetection {
	...
	private static void CreateCommunities(List<Edge> listOfEdges, List<Integer> leaders, List<Integer> members, List<Integer> orbiters) throws IOException {
		// TODO Auto-generated method stub
		...
		if(i > 100){
			for(Entry<Integer,Integer> entry:communities.entrySet()){
				ListIterator<Integer> connectedNodesItr = getListOfNodesConnected(entry.getKey(), listOfEdges).listIterator();
				while(connectedNodesItr.hasNext()){
					...
					if(communities.containsKey(connected)){
						...
						if(null != e1){
							...
							e1 = new Edge(new Node(entry.getValue()),new Node(communities.get(connected)), 1);
							ArrayList<Integer> listOfConnectedComms = new ArrayList<Integer>();
							listOfConnectedComms.add(communities.get(connected));
	...
	private static void WriteCommunitiesInJSON(List<Edge> listOfEdges, Map<Integer, Integer> communities, int i) throws IOException {
		// TODO Auto-generated method stub
        ...
        for(Entry<Integer,Integer> entry:list){
			bw.write("\t\t{\"name\":"+entry.getKey()+",\"group\":"+entry.getValue()+"},"+"\n");
		}
		bw.write("\t],"+"\n");
		bw.write("\t\"links\":["+"\n");
	...
	private static Edge containsEdge(ArrayList<Edge> compressedEdgeList,
		...
		// TODO Auto-generated method stub
		ListIterator<Edge> compressedEdgeListItr = compressedEdgeList.listIterator();
	...
	private static List<Integer> getListOfNodesConnected(int consideredNode, List<Edge> listOfEdges) {
		// TODO Auto-generated method stub
